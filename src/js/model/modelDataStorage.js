...
const adding_authentication = {
  title: 'Authentication & Authorization in REST APIs',
  ...
  sections: [
    ...
    },
    {
      ...
      tooltips: [
        ...
        <ul>There are three main tools that developers use to make sure that only authorised users can access a server and its API:
          <li>1. <b>API Keys</b>;</li>
          <li>2. <b>JSON Web Tokens (JWTs)</b>;</li>
          <li>3. <b>OAuth Tokens</b>.</li>
        </ul>
        `,
        `<h3>Authentication methods</h3>
        <ul>Authentication is typically implemented using various methods such as:
          <li>1. <b>Username and Password</b>: this is the most common form of authentication where users provide a unique username and a secret password that they have previously set up. The server compares these credentials with those stored in its database to authenticate the user.</li>
          <li>2. <b>Token-based Authentication</b>: in this method, <i>a token is generated by the server</i> and sent to the client upon successful authentication. The client then includes this token in subsequent requests to access protected resources. Tokens can be in the form of <i><u>JSON</u> Web Tokens (JWT)</i>, <i><u>OAuth</u> tokens</i>, or <i><u>session</u> tokens</i>.</li>
          <li>3. <b>Multi-factor Authentication (MFA)</b>: MFA adds an extra layer of security by requiring users to provide two or more forms of verification before granting access. This could include something the user knows (like a password), something the user has (like a smartphone or token generator), or something the user is (like biometric data).</li>
        </ul>
        `,
    ...
    {
      sectionTitle: 'Token-Based Authentication: API Keys',
      sectionSource: '',
      highlights: {
        highlight2: ['API Keys'],
      },
      tooltips: [
        `<p>API keys are <i><b>unique codes</b> used to identify and authenticate requests made by a user</i> or an application to an API (Application Programming Interface). They serve as a <i><b>security mechanism</b> to ensure that only authorized users or applications can access certain API functionalities</i>.</p>
        <p>Sometimes, these API keys are public, so anyone can view the API key for some application. But usually you want to keep API keys private, by making sure that they're available only in the server and not in your frontend JavaScript application.</p>
        <p>NOTE: <i>API keys are usually used when the users of your API are developers</i> that are trying to enable some functionality in their application that makes use of the features that you've already exposed in your API.</p>
        `,
        `<h3>What Are API Keys?</h3>
        <p><i>Unique Identifiers</i>: API keys are usually <i>long strings of characters and numbers</i> that are <b>generated by the service providing the API</b>.</p>
        <p><i>Authentication Tool</i>: When a request is sent to an API, <b>the API key is included in the request header or as a query parameter</b>. The server checks the key to verify the identity of the requester.</p>
        <p><i>Access Control</i>: API keys can <i>limit access to specific parts of an API</i> or <i>set usage limits to prevent abuse</i>, such as restricting the number of requests per day.</p>`,
        `<h3>What Are API Keys Used For?</h3>
        <p><i>Access Control</i>: They <b>allow developers to grant or restrict access to specific parts of an API</b>. For example, a free tier of an API might provide limited access, while a premium tier allows full access.</p>
        <p><i>Usage Tracking</i>: API keys enable service providers to monitor and log the usage of their API, which helps in analyzing traffic, detecting abuse, and billing users accordingly.</p>
        <p><i>Rate Limiting</i>: API keys can be used to enforce rate limits, ensuring that a single user or application does not overload the API by making too many requests in a short period.</p>
        <p><i>Security</i>: <b>While not the most secure method, API keys provide a basic level of security by ensuring that only authorized requests are processed.</b> They are often used alongside other security measures like OAuth tokens or IP whitelisting.</p>
        <p><i>Customizing Responses</i>: API keys can be tied to specific settings or preferences, allowing an API to customize responses based on the user's plan or profile.</p>`,
      ],
    },
    {
      sectionTitle: 'Token-Based Authentication: JSON Web Token (JWT)',
      sectionSource: '',
      highlights: {
      ...
      tooltips: [
        ...
        <ul>Assuming we already have a registered user in our database, this is how a user logs into the app:
          <li>1. The user's client starts by making a POST request with the username/email and the password;</li>
          <li>2. The server then checks if the user exists and if the password is correct. If so, a unique JWT Token for only that user is created using a secret string that is stored on a server;</li>
        ...
        <p><img src="../../src/img/token_5.jpg"/></p>
        `,
        `<h3>Creating JSON Web Tokens (JWTs)</h3>
...
const createSendToken = (user, statusCode, req, res) => {
  ...
  res.status(statusCode).json({
    ...
    {
      sectionTitle:
        'Token-Based Authentication: OAuth Token with Google and Passport.js',
      sectionSource: '',
      highlights: {
        highlight1: ['Passport.js'],
        highlight2: ['OAuth Token'],
      },
      tooltips: [
        `<p>One of the most widely used approaches in implementing authentication in REST APIs, used by everything from Google to Facebook to GitHub is what's called OAuth. What's OAuth? It's an authentication standard that enabled things like Social Sign In to become so widespread.<p>
        <p>OAuth defines the flow or process that goes on behind the scenes when you log on to a site like Google.</p>
        <p><img src="../../src/img/oauth_1.jpg"/></p>
        <p>Why add this complexity when you can just put in a password to access some data? The problem that OAuth tries to solve is that <i>we want to let a user access some application (some web application) without necessarily putting their password into that unknown untrusted sites</i>. So following the above flow, <i>the authorization server (in our case Google) will give you back an access token</i>, which you can then use to access data inside of your application. So you never had to put your password into the web application to can access resourses on that app.</p>
        <pre><code>
const express = require('express');
<i>const passport = require(<b>'passport'</b>);
const { Strategy } = require(<b>'passport-google-oauth20'</b>);</i>
require('dotenv').config();

const config = {
  <i>CLIENT_ID: process.env.CLIENT_ID,
  CLIENT_SECRET: process.env.CLIENT_SECRET,</i>
};

const <i>AUTH_OPTIONS</i> = {
  callbackURL: '/auth/google/callback',
  clientID: config.CLIENT_ID,
  clientSecret: config.CLIENT_SECRET,
};

function <i>verifyCallback</i>(accessToken, refreshToken, profile, done) {
  console.log('Google profile', profile);
  done(null, profile);
};

<b>passport.use(new Strategy(AUTH_OPTIONS, verifyCallback));</b>

const app = express();

<i>app.use(<b>passport.initialize()</b>);</i>
...
const cookieSession = require(<b>'cookie-session'</b>);
...
// Passport.js exposes a logout function on the request object; req.logout() can be called on any route
...
<i>const isAuth = require('../middleware/is-auth');</i>
